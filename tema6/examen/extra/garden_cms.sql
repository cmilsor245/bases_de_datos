...
--
-- Host: 127.0.0.1:3306
-- Generation Time: May 10, 2023 at 04:34 PM
...
-- Procedures
--
DROP PROCEDURE IF EXISTS `estadisticas_pedidos`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `estadisticas_pedidos` ()   begin
...
DROP PROCEDURE IF EXISTS `mostrar_compras_cliente`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `mostrar_compras_cliente` (IN `codigo_cliente` INT)   begin
...
DROP PROCEDURE IF EXISTS `mostrar_datos_cliente`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `mostrar_datos_cliente` (IN `codigo_cliente` INT)   begin
...
DROP PROCEDURE IF EXISTS `obtener_total_euros_pedido`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `obtener_total_euros_pedido` (IN `codigo_pedido` INT, IN `limite_total` DECIMAL(15,2), OUT `total_euros` DECIMAL(15,2))   begin
...
-- Functions
--
DROP FUNCTION IF EXISTS `porcentaje_limite_credito`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `porcentaje_limite_credito` (`codigo_cliente` INT) RETURNS DECIMAL(6,2)  begin
...
CREATE TABLE IF NOT EXISTS `cliente` (
  `codigo_cliente_cms` int NOT NULL,
  ...
  PRIMARY KEY (`codigo_cliente_cms`),
...
INSERT INTO `cliente` (`codigo_cliente_cms`, `nombre_cliente`, `nombre_contacto`, `apellido_contacto`, `telefono`, `fax`, `linea_direccion1`, `linea_direccion2`, `ciudad`, `region`, `pais`, `codigo_postal`, `codigo_empleado_rep_ventas`, `limite_credito`) VALUES
...
--
-- Stand-in structure for view `clientes_limite_credito`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `clientes_limite_credito`;
CREATE TABLE IF NOT EXISTS `clientes_limite_credito` (
`apellido_contacto` varchar(30)
,`ciudad` varchar(50)
,`codigo_cliente_cms` int
...
CREATE TABLE IF NOT EXISTS `detalle_pedido` (
  `codigo_pedido_cms` int NOT NULL,
  `codigo_producto_cms` varchar(15) NOT NULL,
  `cantidad` int NOT NULL,
  `precio_unidad` decimal(15,2) NOT NULL,
  `numero_linea` smallint NOT NULL,
  PRIMARY KEY (`codigo_pedido_cms`,`codigo_producto_cms`),
  KEY `codigo_producto_cms` (`codigo_producto_cms`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
INSERT INTO `detalle_pedido` (`codigo_pedido_cms`, `codigo_producto_cms`, `cantidad`, `precio_unidad`, `numero_linea`) VALUES
(1, 'FR-67', 10, '70.00', 3),
(1, 'OR-127', 40, '4.00', 1),
(1, 'OR-141', 25, '4.00', 2),
(1, 'OR-241', 15, '19.00', 4),
...
CREATE TABLE IF NOT EXISTS `empleado` (
  `codigo_empleado_cms` int NOT NULL,
  ...
  PRIMARY KEY (`codigo_empleado_cms`),
...
INSERT INTO `empleado` (`codigo_empleado_cms`, `nombre`, `apellido1`, `apellido2`, `extension`, `email`, `codigo_oficina`, `codigo_jefe`, `puesto`) VALUES
...
INSERT INTO `logs` (`id`, `info`, `fecha`) VALUES
(1, 'Se insertó el producto con el ID OR-300 , de nombre <Geranium columbinum L.>, cuya gama es <Ornament', '2023-05-10 17:41:07'),
(2, 'Se insertó el producto con el ID OR-301 , de nombre <Geranium dissectum L.>, cuya gama es <Ornamenta', '2023-05-10 17:41:07'),
(3, 'Se insertó el producto con el ID OR-302 , de nombre <Geranium dolomiticum L.>, cuya gama es <Ornamen', '2023-05-10 17:41:07'),
(4, 'Se insertó el producto con el ID OR-303 , de nombre <Geranium lucidum L.>, cuya gama es <Ornamentale', '2023-05-10 17:41:07'),
(5, 'Se insertó el producto con el ID OR-304 , de nombre <Geranium molle L.>, cuya gama es <Ornamentales>', '2023-05-10 17:41:07'),
(6, 'Se insertó el producto con el ID FR-300 , de nombre <Kiwi>, cuya gama es <Frutales>, tiene unas dime', '2023-05-10 17:41:07'),
(7, 'Se insertó el producto con el ID FR-301 , de nombre <Mango>, cuya gama es <Frutales>, tiene unas dim', '2023-05-10 17:41:07'),
(8, 'Se insertó el producto con el ID FR-302 , de nombre <Aguacate>, cuya gama es <Frutales>, tiene unas ', '2023-05-10 17:41:07');
...
CREATE TABLE IF NOT EXISTS `pago` (
  ...
  PRIMARY KEY (`codigo_cliente_cms`,`id_transaccion`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
INSERT INTO `pago` (`codigo_cliente_cms`, `forma_pago`, `id_transaccion`, `fecha_pago`, `total`) VALUES
...
CREATE TABLE IF NOT EXISTS `pedido` (
  ...
  `comentarios` text,
  `codigo_cliente_cms` int NOT NULL,
  PRIMARY KEY (`codigo_pedido_cms`),
  KEY `codigo_cliente_cms` (`codigo_cliente_cms`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
INSERT INTO `pedido` (`codigo_pedido_cms`, `fecha_pedido`, `fecha_esperada`, `fecha_entrega`, `estado`, `comentarios`, `codigo_cliente_cms`) VALUES
...
CREATE TABLE IF NOT EXISTS `producto` (
  ...
  PRIMARY KEY (`codigo_producto_cms`),
  KEY `gama` (`gama`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
INSERT INTO `producto` (`codigo_producto_cms`, `nombre`, `gama`, `dimensiones`, `proveedor`, `descripcion`, `cantidad_en_stock`, `precio_venta`, `precio_proveedor`) VALUES
...
DELIMITER $$
CREATE TRIGGER `check_price` BEFORE INSERT ON `producto` FOR EACH ROW begin
...
DELIMITER $$
CREATE TRIGGER `entrada_nuevos_productos` AFTER INSERT ON `producto` FOR EACH ROW begin
...
DELIMITER $$
CREATE TRIGGER `limit_stock` BEFORE UPDATE ON `producto` FOR EACH ROW begin
...
--
-- Stand-in structure for view `vista_pedidos_pendientes`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `vista_pedidos_pendientes`;
CREATE TABLE IF NOT EXISTS `vista_pedidos_pendientes` (
`ciudad` varchar(50)
,`estado` varchar(15)
,`fecha_esperada` date
,`fecha_pedido` date
,`nombre_cliente` varchar(50)
,`precio_total` decimal(47,2)
);

-- --------------------------------------------------------

--
-- Structure for view `clientes_limite_credito`
--
DROP TABLE IF EXISTS `clientes_limite_credito`;

DROP VIEW IF EXISTS `clientes_limite_credito`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `clientes_limite_credito`  AS SELECT `c`.`codigo_cliente_cms` AS `codigo_cliente_cms`, `c`.`nombre_cliente` AS `nombre_cliente`, `c`.`nombre_contacto` AS `nombre_contacto`, `c`.`apellido_contacto` AS `apellido_contacto`, `c`.`telefono` AS `telefono`, `c`.`fax` AS `fax`, `c`.`linea_direccion1` AS `linea_direccion1`, `c`.`linea_direccion2` AS `linea_direccion2`, `c`.`ciudad` AS `ciudad`, `c`.`region` AS `region`, `c`.`pais` AS `pais`, `c`.`codigo_postal` AS `codigo_postal`, `c`.`codigo_empleado_rep_ventas` AS `codigo_empleado_rep_ventas`, `c`.`limite_credito` AS `limite_credito` FROM `cliente` AS `c` WHERE (`c`.`limite_credito` > (select (avg(`cliente`.`limite_credito`) * 1.05) from `cliente`)) ORDER BY `c`.`ciudad` DESC, `c`.`region` ASC  ;

-- --------------------------------------------------------

--
-- Structure for view `vista_pedidos_pendientes`
--
DROP TABLE IF EXISTS `vista_pedidos_pendientes`;

DROP VIEW IF EXISTS `vista_pedidos_pendientes`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_pedidos_pendientes`  AS SELECT `cliente`.`nombre_cliente` AS `nombre_cliente`, `cliente`.`ciudad` AS `ciudad`, `pedido`.`fecha_pedido` AS `fecha_pedido`, `pedido`.`fecha_esperada` AS `fecha_esperada`, `pedido`.`estado` AS `estado`, sum((`detalle_pedido`.`cantidad` * `detalle_pedido`.`precio_unidad`)) AS `precio_total` FROM ((`cliente` join `pedido` on((`cliente`.`codigo_cliente_cms` = `pedido`.`codigo_cliente_cms`))) join `detalle_pedido` on((`pedido`.`codigo_pedido_cms` = `detalle_pedido`.`codigo_pedido_cms`))) WHERE (`pedido`.`estado` <> 'Entregado') GROUP BY `pedido`.`codigo_pedido_cms` ORDER BY `precio_total` AS `DESCdesc` ASC  ;
COMMIT;
